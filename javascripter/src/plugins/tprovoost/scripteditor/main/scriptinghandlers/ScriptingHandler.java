 ...
 * 
 */
public abstract class ScriptingHandler extends JScrollPane implements KeyListener {

	/** Default serialVersionUID because this item extends {@link JScrollPane}. */
	...
	protected HashMap<String, Class<?>> localVariables = new HashMap<String, Class<?>>();
	...
	protected HashMap<String, Class<?>> engineVariables = new HashMap<String, Class<?>>();
	protected HashMap<String, Class<?>> engineFunctions = new HashMap<String, Class<?>>();
	protected ArrayList<String> engineDeclaredImports = new ArrayList<String>();
	protected ArrayList<String> engineDeclaredImportClasses = new ArrayList<String>();
	...
	public HashMap<String, Class<?>> getLocalVariables() {
		...
		if (advanced) {
			ThreadUtil.bgRun(new Runnable() {
				...
				public void run() {
					...
					((IcyCompletionProvider)provider).findAllMethods(engineFunctions, engineTypesMethod, engine, frame);
			...
			for (PluginDescriptor pd : PluginLoader.getPlugins()) {
				try {
					...
					((IcyCompletionProvider)provider).findBindingsMethods(engineFunctions, engineTypesMethod, engine, clazz2);
	...
	private void interpret(String s, boolean autocompilation, boolean exec) {
		...
		try {
			...
			if (exec && isCompilationOk()) {
				...
				engineVariables.putAll(localVariables);
				engineFunctions.putAll(localFunctions);
				engineDeclaredImportClasses.addAll(scriptDeclaredImportClasses);
				engineDeclaredImports.addAll(scriptDeclaredImports);
			}
