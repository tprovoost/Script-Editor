...
public class JSScriptingHandler extends ScriptingHandler {
	...
	public void installDefaultLanguageCompletions(String language) throws ScriptException {
		...
		engineHandler.getEngineFunctions().put("getSequence()", Sequence.class);
		...
		provider.addCompletion(c);
		engineHandler.getEngineFunctions().put("getImage()", IcyBufferedImage.class);
	 ...
	 */
	private void registerVariables(String text, Node n, ScriptOrFnNode root, int commandStartOffset, int commandEndOffset)
			throws ScriptException {
		// register current
			...
			while (child != null) {
				...
				if (n == root) {
					...
					while (commandStartOffset < text.length()
							&& ((charvalue = text.charAt(commandStartOffset)) == '\n' || charvalue == ' ' || charvalue == '\t'
									|| charvalue == '{' || charvalue == '}'))
						commandStartOffset++;
	 ...
	 * @throws ScriptException
	 */
	private Completion generateCompletion(Node n, ScriptOrFnNode root, String text, int commandStartOffset, int commandEndOffset)
			throws ScriptException {
		switch (n.getType()) {
		case Token.VAR:
			if (n.getFirstChild() != null && n.getFirstChild().getType() == Token.NAME) {
				...
				if (type != null)
					...
					addVariableDeclaration(c.getName(), getRealType(resNode, text, start, commandEndOffset), commandStartOffset);
				return c;
			}
			break;
		case Token.SETNAME: {
		...
		if (n.getFirstChild() != null && n.getFirstChild().getType() == Token.GETPROP) {
			...
			if (match.find(0)) {
				...
				try {
					...
					// variable
					clazz = getVariableDeclaration(className);
					ScriptEngineHandler engineHandler = ScriptEngineHandler.getEngineHandler(engine);

					if (clazz == null)
						clazz = engineHandler.getEngineVariables().get(className);
					if (clazz == null)
						// class
					...
					if (argsString.isEmpty()) {
						...
						clazzes = getGenericNumberTypes(text, clazz, firstCall.substring(lastDot + 1, idxP1), clazzes, commandStartOffset,
								commandEndOffset);
					}
					...
					int idx = text.indexOf(firstCall);
					IcyFunctionBlock fb = new IcyFunctionBlock(firstCall.substring(lastDot + 1, idxP1), idx, idx + decal, returnType);
					blockFunctions.add(fb);

					// iterate over the next functions, based on the returnType
					while (match.find(decal) && !(firstCall = match.group()).isEmpty()) {
						if (returnType == void.class)
							throw new ScriptException("Void return, impossible to call something else on it.", null, findLineContaining(
									text, commandStartOffset));
						idxP1 = firstCall.indexOf('(');
						idxP2 = firstCall.indexOf(')');
						decal += idxP2 + 1;
						...
						returnType = m.getReturnType();
						fb = new IcyFunctionBlock(firstCall.substring(0, idxP1), commandStartOffset, commandStartOffset + decal,
								returnType);
						blockFunctions.add(fb);
					}
					return returnType;
			...
			if (res == null)
				res = ScriptEngineHandler.getEngineHandler(engine).getEngineFunctions().get(firstChild.getString() + args);
			if (res == null)
	 ...
	 * @return
	 */
	private Class<?>[] getGenericNumberTypes(String s, Class<?> clazz, String function, Class<?>[] argsClazzes, int commandStartOffset,
			int commandEndOffset) {
