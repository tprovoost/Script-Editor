...
public class JSScriptingHandler6 extends ScriptingHandler
{
    ...
    private Class<?> resolveNewType(Node n, String text) throws ScriptException
    {
        ...
        if (clazz == null)
            throw new ScriptException("Type: " + type + " is undefined. Please check imports.", null,
        ...
        switch (currentChild.getType())
        {
            case Token.GETPROP:
                return buildRecursiveNew(currentChild.getFirstChild()) + "."
                        + buildRecursiveNew(currentChild.getLastChild());
            case Token.STRING:
            case Token.NAME:
    ...
    private Class<?> resolveCallType(Node n, String text, boolean noerror) throws ScriptException
    {
        if (n.getFirstChild() != null && n.getFirstChild().getType() == Token.GETPROP)
        {
            ...
            if (match.find(0))
            {
                ...
                try
                {
                    ...
                    if (clazz == null)
                        throw new ScriptException("Unknown class: " + classNameOrFunctionNameOrVariable, null,
                                findLineContaining(text));
                    ...
                    if (DEBUG)
                        System.out.println("function created: " + commandStartOffset + " "
                                + text.substring(commandStartOffset));
                    IcyFunctionBlock fb = new IcyFunctionBlock(firstCall.substring(lastDot + 1, idxP1),
                            commandStartOffset, returnType);
                    ...
                    while (match.find(decal) && !(firstCall = match.group()).isEmpty())
                    {
                        ...
                        else
                        {
                            ...
                            clazzes = getGenericNumberTypes(text, returnType, firstCall.substring(lastDot + 1, idxP1),
                                    clazzes);
                        }
                        String call = firstCall.substring(lastDot + 1, idxP1);
                        if (call.contentEquals("newInstance"))
                        ...
                        if (DEBUG)
                            System.out.println("function created: " + commandStartOffset
                                    + text.substring(commandStartOffset));
                        fb = new IcyFunctionBlock(firstCall.substring(lastDot + 1, idxP1), commandStartOffset,
                                returnType);
                        blockFunctions.put(commandStartOffset, fb);
                    }
                    return returnType;
                ...
                {
                    throw new ScriptException("Class Not Found: " + e.getLocalizedMessage(), null,
                            findLineContaining(text));
                }
        ...
        else
        {
            ...
            if (firstChild.getType() != Token.GETELEM)
            {
                ...
                if (res == null)
                    res = ScriptEngineHandler.getEngineHandler(getEngine()).getEngineFunctions()
                            .get(firstChild.getString()); // +
                // args);
                if (res == null && !noerror)
                    throw new ScriptException("Var Detection: " + firstChild.getString() + args + " does not exist.",
                            null, findLineContaining(text));
                return res;
            }
            else
            {
                ...
                try
                {
                    ...
                    if (DEBUG)
                        System.out.println("function created: " + commandStartOffset + " "
                                + text.substring(commandStartOffset));
                    IcyFunctionBlock fb = new IcyFunctionBlock(s.substring(lastDot + 1, idxP1), commandStartOffset,
                            returnType);
                    blockFunctions.put(commandStartOffset, fb);
                    return returnType;
                }
    ...
    private Class<?>[] getGenericNumberTypes(String s, Class<?> clazz, String function, Class<?>[] argsClazzes)
    {
        ...
        String fullCommand;
        if (commandStartOffset >= 0 && commandStartOffset < s.length() && commandEndOffset >= 0
                && commandEndOffset <= s.length())
     ...
     * @throws ScriptException
     */
    private String buildFunctionRecursive(String callName, Node n, String text, boolean functionNext)
            throws ScriptException
    {
        if (n != null)
        {
